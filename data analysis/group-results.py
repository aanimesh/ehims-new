...
group_stats = r'results/channel-{}.txt'.format(channel)
group_info = r'results/channel-{}.xlsx'.format(channel)
plt_path = r'results/channel-{}.png'.format(channel)


def get_stats():
    file = open(group_stats, 'w+')
...
group = json.load(open(file_path, 'r', encoding='utf8'))
...
for key, msg in group['messages'].items():
    ...
    if 'msg_parent' not in msg.keys():
        root_msg.append(key)
    relation.extend([(key, cmsg.strip()) for cmsg in msg['children'].replace(' ', '').split(',')])
...
max_height = len(nx.dag_longest_path(G_msg))
...
df['centrality'] = pd.Series(list(map(lambda m: centrality[m], mid)), index=df.index)
df['in_centrality'] = pd.Series(list(map(lambda m: in_centrality[m], mid)), index=df.index)
df['out_centrality'] = pd.Series(list(map(lambda m: out_centrality[m], mid)), index=df.index)
...
for node in mid:
    if node:
        closeness_centrality.append(nx.closeness_centrality(G_msg, node))
...
with pd.ExcelWriter(group_info) as writer:
    df.to_excel(writer, sheet_name='messages')
    df1.to_excel(writer, sheet_name='participants')
...
nx.draw(G_msg, pos=pos, node_size=150, node_color=msg_color, cmap=plt.cm.Paired)
plt.show()
